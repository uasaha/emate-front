...
function viewReplyAn(reply) {
  ...
  if (document.getElementById('replyDiv') == null) {
    ...
    document.getElementById("momContentsNo").value = document.getElementById(
        "contentsNo-an").value;
...
function viewReplyMe(reply) {
  ...
  if (document.getElementById('replyDiv') == null) {
    ...
    replyDiv.innerHTML = '<form id="child-form" action="/comments/member" method="post">\n'
        ...
        +
        '                        <input class="display-none" name="memberNo" value="'
        + memberNo + '">\n' +
        '                        <div style="display: inline-block">' +
        '                            <img src="/static/img/favicon/favicon-32x32.png" class="comment-nick-image">'
        +
        '                            <span class="color-white comment-nick">'
        + memberNick + '</span>\n' +
        '                        </div>' +
        '                    </div>\n' +
    ...
    mom.appendChild(replyDiv);
    mom.style.borderBottom = "0px solid white";
    document.getElementById("momContentsNo-me").value = document.getElementById(
        "contentsNo-me").value;
...
function totalTesting(idStr) {
  return isValidNickname(idStr) && isValidPassword(idStr) && isValidContent(
      idStr);
}
